...
export default class MoneyItemComponent extends React.Component {
    ...
    onRefresh = () => {
        ...
        setTimeout(() => {
            ...
            $.ajax({
                type: 'get',
                async: false,
                url: https.api.tasklistDescManuscript
            }).done(function(res) {
                obj = res;
            })
    ...
    render() {
        const row = (rowData, sectionID, rowID) => {
            ...
            return (
                <Link to={`/taskDetail&${obj.task_id}`}>
                    <Item className="task-item" arrow="empty" style={{
                        overflow: 'hidden'
                    }} multipleLine>
                        <Brief>
                            <b style={{
        ...
        return (
            <div><ListView removeClippedSubviews={true} initialListSize={10} dataSource={this.state.dataSource} renderFooter={() => <div style={{
                textAlign: 'center',
                fontSize: '0.6rem'
            }}>
